defmodule AuthenticatorTest do
  ...
  test "AuthenticationRequest / Response" do
    ...
    {:ok, authenticate_response} = GenServer.call(Authenticator, {:authenticate_request, %AuthenticateRequest{username: "jimmy", password: user.password, token_request_type: "api"}})
    assert authenticate_response.is_authenticated == false
    {:ok, authenticate_response} = GenServer.call(Authenticator, {:authenticate_request, %AuthenticateRequest{username: user.name, password: "NOTMINE", token_request_type: "api"}})
    assert authenticate_response.is_authenticated == false
